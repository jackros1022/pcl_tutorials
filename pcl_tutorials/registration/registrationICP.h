...
int getInteractive_icp()
{
	...
	*cloud_tr = *cloud_icp;  // 将cloud_icp变量备份
	...
	icp.align(*cloud_icp);
	icp.setMaximumIterations(5);  //当再次调用.align ()函数时，我们设置该变量为1。
	...
	// 设置初始点云为白色
	pcl::visualization::PointCloudColorHandlerCustom<PointT> cloud_in_color_h(cloud_in, (int)255 * txt_gray_lvl, (int)255 * txt_gray_lvl, (int)255 * txt_gray_lvl);//赋予显示点云的颜色
